...
- The reverse direction as well: a merged track can be split back into its two layers for a disc, through the existing subTrack= mechanism, with the base layer on one PID and the enhancement layer on the other. Two disc layers merged into one Matroska track and split again produce a Blu-ray that is byte identical to authoring it straight from those layers. A single layer file has no enhancement layer to separate and is refused with an explanation rather than producing an empty second track.
