...
--dv-profile          Matroska output only, and only for a dual layer Dolby Vision
                      ...
                      both layers, reversible. 8.1 converts the Dolby Vision metadata
                      so the file plays as single layer profile 8.1, which many more
                      devices accept, while the disc's enhancement layer still travels
                      inside the track where decoders skip it. The file is therefore
                      NOT smaller than a profile 7 one; that is the price of being
                      able to rebuild the disc from it later.
                      8.1 needs the libdovi library (dovi.dll on Windows) beside
                      tsMuxeR. It is published with dovi_tool for 64 bit Windows;
                      other platforms build it from source. Without it the option is
                      refused before muxing starts.
)help"
